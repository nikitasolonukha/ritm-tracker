...
Проверяемый кодовый commit: `29a6f6c` (`feat: add workout set editing controls`).
...
- Production deployment текущего commit: `dpl_9uebkQGbVjWNdyKwhqAhbUU4bmtt` READY; реальный Telegram job/callback по-прежнему требует отдельного owner-approved теста.
