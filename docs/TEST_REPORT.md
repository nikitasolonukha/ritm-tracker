...
Кодовый commit: `40f7a40` (`feat: resolve cross-device sync conflicts`).
...
- Текущий подтвержденный production deployment: `dpl_8vw1Cj5xT3VhDMqX2YLgxSTNA5oM` (READY), alias `https://ritm-tracker.vercel.app/`.
...
- При обнаружении revision-конфликта UI предлагает оставить локальную или серверную копию; выбор локальной копии выполняет повторный owner-scoped PUT с актуальной revision. Фактический сценарий на двух устройствах пока не запускался.
