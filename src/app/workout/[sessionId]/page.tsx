...
import { ArrowLeft, Check, ChevronRight, Clock, Pause, Play, RotateCcw } from "lucide-react";
...
export default function ActiveWorkoutPage({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  useEffect(() => { const id = window.setInterval(() => { if (!paused) setNow(new Date().toISOString()); }, 1000); return () => window.clearInterval(id); }, [paused]);
  ...
  const record = () => {
    ...
    update((previous) => {
      ...
      return { ...next, workouts: next.workouts.map((item) => item.id === workout.id ? result.workout : item), workoutCommands: result.commands, activeTimer: result.activeTimer, outbox: [...next.outbox, { id: `out-${session.id}-${currentSet.id}`, entityId: `${workout.id}:${exercise.id}:${currentSet.id}`, type: "workout.set.completed", createdAt: new Date().toISOString(), status: "pending", version: 1 }] };
    });
    setReps("");
  };
  ...
  return <main className="workoutMode"><header className="workoutTop"><Link href="/workouts" aria-label="Свернуть"><ArrowLeft size={20} /></Link><span className="workoutTimer"><Clock size={16} /> {formatTime(elapsed)}</span><button className="iconButton" aria-label={paused ? "Продолжить" : "Пауза"} onClick={() => setPaused((value) => !value)}>{paused ? <Play size={19} /> : <Pause size={19} />}</button></header>
    ...
    <section className="activeExercise"><p className="eyebrow">Текущее упражнение</p><h1>{exercise.name}</h1><p className="muted">{exercise.settings ?? "Рабочий подход"}</p>
      {rest && !rest.expired && <div className="restState"><span>Отдых</span><strong>{formatTime(rest.remainingSec)}</strong><small>Следующий подход {Math.min(setIndex + 2, exercise.sets.length)} из {exercise.sets.length}</small><button className="secondary" onClick={() => update((previous) => ({ ...previous, activeTimer: null }))}><Play size={16} /> Пропустить отдых</button></div>}
      {(!rest || rest.expired) && !completeExercise && <div className="setState"><p className="eyebrow">Подход {Math.max(1, setIndex + 1)} из {exercise.sets.length}</p><div className="workWeight"><strong>{currentSet?.weightKg ?? "—"}</strong><span>{currentSet?.weightMode === "per-hand" ? "кг на сторону" : "кг · режим уточнить"}</span></div><label>Фактические повторения<input autoComplete="off" inputMode="numeric" value={reps || (currentSet?.reps ?? "")} onChange={(event) => setReps(event.target.value)} placeholder={`План: ${currentSet?.reps ?? "—"}`} /></label>{error && <p className="fieldError" role="alert">{error}</p>}<button className="primary recordButton" onClick={record}><Check size={20} /> Записать {reps || currentSet?.reps || "подход"}</button></div>}
    ...
    </section><button className="secondary workoutCancel" onClick={() => router.push("/workouts")}><RotateCcw size={16} /> Свернуть</button></main>;
}
