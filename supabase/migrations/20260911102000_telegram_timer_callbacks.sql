...
begin
  if p_telegram_user_id is null or nullif(trim(p_command_key), '') is null
     ...
     or p_source_version < 1 or p_action not in ('cancel', 'reschedule') then
  ...
  select max(version) into latest_version
   ...
   where user_id = linked_user_id and entity_id = p_entity_id;
  ...
  if p_action = 'reschedule' then
    select id into job_id
     ...
     where user_id = linked_user_id and source_entity_id = p_entity_id
       and source_version = p_source_version
       and status in ('pending', 'leased', 'failed', 'unknown')
     limit 1;
    if job_id is not null then
      return jsonb_build_object('status', 'duplicate', 'job_id', job_id);
    end if;
  end if;

  insert into public.commands(user_id, command_key, command_type, entity_id, payload, version)
    values (linked_user_id, p_command_key, 'workout.timer.' || p_action, p_entity_id, p_payload, p_source_version)
  ...
  if command_id is null then
    ...
    select id into job_id from public.notification_jobs where user_id = linked_user_id and source_entity_id = p_entity_id and source_version = p_source_version;
  ...
  if p_action = 'reschedule' then
    insert into public.notification_jobs(user_id, source_entity_id, source_version, due_at, expires_at, message)
      values (linked_user_id, p_entity_id, p_source_version, now() + interval '30 seconds', now() + interval '5 minutes', 'Ритм: отдых продлен на 30 секунд.')
